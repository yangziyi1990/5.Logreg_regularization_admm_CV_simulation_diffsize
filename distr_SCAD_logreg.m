function [z, history] = distr_SCAD_logreg(A, b, mu, N, rho, alpha, n1, n2, n3)

% distr_SCAD_logreg   Solve distributed SCAD regularized logistic regression
%
% [x, history] = distr_SCAD_logreg(A, b, mu, N, rho, alpha)
%
...
%
% This solves the SCAD regularized logistic regression problem. It uses
% a custom Newton solver for the x-step. This version solves a distributed
% version of SCAD regularized logistic regression.
%
...
function z = shrinkage(S, lambda)
    ...
    for j=1:len
        ...
        elseif abs(S(j)) <= 2*lambda 
            z(j)=s*(abs(S(j))-lambda);    
        elseif abs(S(j)) <= alpha*lambda
            z(j)=s*(abs(S(j))-alpha*lambda/(alpha-1))/(1-1/(alpha-1));
        else
            z(j)=S(j);
        end
    end
