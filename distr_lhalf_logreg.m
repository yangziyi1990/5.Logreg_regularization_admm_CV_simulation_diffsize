function [z, history] = distr_lhalf_logreg(A, b, mu, N, rho, alpha, n1, n2, n3)

% distr_lhalf_logreg   Solve distributed Lhalf regularized logistic regression
%
% [x, history] = distr_lhalf_logreg(A, b, mu, N, rho, alpha)
%
% solves the following problem via ADMM:
%
%   minimize   sum( log(1 + exp(-b_i*(a_i'w + v)) ) + m*mu*norm(w,1/2)
%
% where A is a feature matrix and b is a response vector. The scalar m is
% the number of examples in the matrix A.
%
% This solves the Lhalf regularized logistic regression problem. It uses
% a custom Newton solver for the x-step. This version solves a distributed
% version of Lhalf regularized logistic regression.
%
...
function z = shrinkage(s, lambda)
    len=length(s);
    for i=1:len
        if abs(s(i)) > ((3/4)*(lambda^(2/3)))
            phi = acos(lambda/8*((abs(s(i))/3)^(-1.5)));
            z(i) = real(2/3*s(i)*(1 + cos(2/3*(pi - phi)))); 
        else
            z(i)=0;
        end
    end
